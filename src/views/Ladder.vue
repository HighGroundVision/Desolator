                  ...
                  <div class="w3-card s12 w3-row w3-round w3-padding">
                    ...
                    <div class="w3-col s8">
                      <img
                        ...
                        class="profile-icon"
                        src="http://cdn.akamai.steamstatic.com/steamcommunity/public/images/avatars/fe/fef49e7fa7e1997310d705b2a6158ff8dc1cdfeb_full.jpg"
                      />
                      <img v-else class="profile-icon" :src="item.avatar" />
                      <h2 style="display: inline; vertical-align: 50%;">
                        <router-link
                          style="text-decoration: none;"
                          :to="'/player/' + item.accountId"
                        >
                    ...
                    <div class="w3-col s2">
                      <b>Rating:</b>
                      <span>{{Math.round(item.ranking)}}</span>
                      <br />
                      <b>Matches:</b>
  ...
  methods: {
    async loadRegionData() {
      ...
      var response = await axios.get(
        process.env.VUE_APP_BASE_API + "api/regions"
      );
    ...
    async loadLatterData() {
      this.loading = true;
      var response = await axios.get(
        process.env.VUE_APP_BASE_API + "api/leaderboard/region/" + this.region
      );
